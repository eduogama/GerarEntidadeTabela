...
namespace GerarCodigoBanco
{
    public partial class FrmServices : Form
    {
        ...
        private void FrmServices_Load(object sender, EventArgs e)
        {
            ...
            TxtNameSpace.Text = String.Format("{0}.{1}", config.Sistema, config.NamespaceService);
        }
            ...
            if (LvwTabelas.Items.Count > 0)
            {
                if (TxtNameSpace.Text != "")
                {
                    foreach (ListViewItem item in LvwTabelas.Items)
                    {
                        ...
                        {
                            ClsGerar.Services(TxtNameSpace.Text, item.SubItems[i].Text);
                        }
                    }
        ...
        private void BtnExecutar_Click(object sender, EventArgs e)
        {
            if (LvwTabelas.SelectedItems.Count > 0)
            {
                if (TxtNameSpace.Text != "")
                {
                    ClsGerar.Services(TxtNameSpace.Text, LvwTabelas.SelectedItems[0].Text);
                    MessageBox.Show("Classe Gerada com Sucesso");
                }
                else
                {
